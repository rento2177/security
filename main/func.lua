...
function p51()
    gg.alert([[
        ...
        ・スクリプトの復号が確認された場合は様子を伺い、システムが突破されそうになった際にアクセス拒否と必要に応じて情報を公開します。

        ・二次配布又は転売など、第三者への譲渡が確認された場合はIDを無効化(アクセス拒否)します。また、同時に購入履歴も削除されます。

        ・サーバーへの攻撃又は侵入はIDの無効化及び情報を公開します。

        当サービスでは不正行為を検知すると自動的にアクセス情報を解析し、管理者へ報告及び情報の送信が行われます。
        情報の公開: 解析システムから受信した不正行為者のアクセス情報をサポートサーバーにて公開すること。

        【システムバージョン】
        Nyanko_x(にゃんこスクリプト)   : 4.1
        SCRTY(セキュリティシステム)    : 1.0
        GGSX(簡易セキュリティシステム) : 2.0
